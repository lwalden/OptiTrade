...
| **Backtesting** | QuantConnect LEAN (Python API) | Institutional-grade options backtesting with 20+ years data |
...
|---|---|---|
| Max risk per trade | 2.5% of NLV ($2,500-$3,750) | Pre-trade check, order rejected if exceeded |
...
**Goal:** Connect to IBKR, execute a single iron condor in paper mode, with paper/live toggle working.
...
**Goal:** QuantConnect LEAN backtesting validation, performance analytics, second broker adapter (Tradier), production deployment on Azure, dashboard.
...
|---|---|---|---|
| 2026-02-19 | Python-primary stack | Ecosystem alignment with ib_async, py_vollib, QC LEAN, MCP servers. Fastest path to production. | C#/.NET (deeper expertise but ecosystem friction), Polyglot (added complexity) |
...
4. **Phase 4 success:** Backtest validates live paper trading results within 2% CAGR; system deployed to production; first live trade executed
